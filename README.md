...
| - | 事件驱动调仓（5轮迭代，最优v5夏普1.52<1.63）|

**核心洞察**：多源数据因子 + 简单等权 + 止损，优于任何"更聪明"的策略/模型叠加——简单就是最好。

### 事件驱动调仓实验（2026-08，五轮迭代，代码为v1之前的版本，下月更新）

系统性验证"事件驱动动态调仓"是否优于"月度固定调仓"，共五轮迭代：

| 版本 | 平均夏普 | 窗口4(2022熊) | 窗口6(2024震荡) |
| :--: | :--: | :--: | :--: |
| **月度固定（基线）** | **1.63** | **+26.84%** | +33.46% |
| v1/v2（止损2只重训 + 到期补满）| 1.52 | -4.83% | +5.27% |
| v3（续约连续2月 + 坏市场补新）| 1.29 | -8.50% | -2.07% |
| v4（熔断：≥4次停15天/≥8次停30天）| 0.61 | -9.10% | +2.28% |
| v5（温和熔断：≥4次停4天，每多1次+1天）| 1.52 | -9.10% | **+77.51%** |

**结论**：
- 事件驱动在**震荡市更强**（v5 窗口6 +77.51% vs 月度 +33.46%），但**熊市必然亏损**（所有版本窗口4 约 -9% vs 月度 +26.84%）
- 综合夏普始终略低于月度固定（最优 1.52 < 1.63），"频繁调仓"在熊市反复止损割肉
- **v5（温和熔断）是最有潜力的方向**：震荡市大幅跑赢，且熔断参数"温和"远优于"狠"（v5 1.52 vs v4 0.61）
- 未来方向：v5 的"好/坏市场判断"目前用"空仓占比"作代理，**将来可用时序模型（regime/波动率）替代**，有望补上熊市短板、超过月度固定

## 交流

acsorceress@gmail.com
